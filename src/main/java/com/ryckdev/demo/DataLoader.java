...
public class DataLoader implements CommandLineRunner {
  ...
  private final RCliente clienteRepository;
    ...
    @Autowired
    public DataLoader(RCliente clienteRepository, RTecnico tecnicoRepository, RServicio servicioRepository, RIncidente incidenteRepository) {
...
// Crear algunos incidentes
        ...
        incidente3.setFechaIngreso(new Timestamp(LocalDate.now().minusDays(1).atStartOfDay(ZoneId.systemDefault()).toInstant().toEpochMilli())); // Convertir LocalDate a Timestamp
        incidente3.setFechaPosibleResolucion(new Timestamp(LocalDate.now().minusDays(1).atStartOfDay(ZoneId.systemDefault()).toInstant().toEpochMilli())); // Convertir LocalDate a Timestamp
        ...
        incidente3.setTiempoResolucion(1); // Tiempo de resolución en horas

        EIncidente incidente4 = new EIncidente();
        incidente4.setDescripcionProblema("Descripción4");
        incidente4.setFechaIngreso(new Timestamp(LocalDate.now().minusDays(1).atStartOfDay(ZoneId.systemDefault()).toInstant().toEpochMilli())); // Convertir LocalDate a Timestamp
        incidente4.setFechaPosibleResolucion(new Timestamp(LocalDate.now().minusDays(1).atStartOfDay(ZoneId.systemDefault()).toInstant().toEpochMilli())); // Convertir LocalDate a Timestamp
        ...
        while (!salir) {
            ...
            switch (opcion) {
                case 1:
                    menuRecursosHumanos(scanner);
                    break;
                case 2:
                    menuAreaComercial(scanner);
                    break;
                case 3:
                    menuMesaDeAyuda(scanner);
                    break;
                case 4:
                    salir = true;
   ...
   private void menuRecursosHumanos(Scanner scanner) {
        ...
        while (!volver) {
            ...
            switch (opcionRH) {
                ...
                case 5:
                    ...
                    System.out.println("Servicio ingresado correctamente.");
                    volver = true;
                    break;
                case 6:
                    volver = true;
    ...
    private void menuAreaComercial(Scanner scanner) {
        boolean volver = false;

        while (!volver) {
            System.out.println("----- Menú de Area Comercial -----");
            System.out.println("1. Ingresar un nuevo Cliente");
            System.out.println("2. Volver");


            int opcionAC = scanner.nextInt();
            scanner.nextLine();
    ...
    private void menuMesaDeAyuda(Scanner scanner) {
        boolean volver = false;

        while (!volver) {
            System.out.println("----- Menú de Mesa de Ayuda -----");
            System.out.println("1. Ingresar un nuevo Incidente");
            System.out.println("2. Volver");

            int opcionMDA = scanner.nextInt();
            scanner.nextLine();
